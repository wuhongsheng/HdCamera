...
class OrientationLiveData(
    ...
    private val listener = object : OrientationEventListener(context.applicationContext) {
        override fun onOrientationChanged(orientation: Int) {
            val rotation = when {
                orientation <= 45 -> Surface.ROTATION_0//默认竖直状态
                orientation <= 135 -> Surface.ROTATION_90//左横屏
                orientation <= 225 -> Surface.ROTATION_180//倒立
                orientation <= 315 -> Surface.ROTATION_270//右横屏
